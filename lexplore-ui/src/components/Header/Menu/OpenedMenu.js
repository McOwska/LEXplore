...
const OpenedMenu = ({ toggleMenu }) => {
  ...
  return (
    <div className={`${styles.openedMenu} ${styles.buttonColors}`}>
      ...
      <button onClick={() => navigate('/languages')} >{intl.formatMessage({ id: "menu.availableLanguages" })}</button>
      <button onClick={() => navigate('/about')}>{intl.formatMessage({ id: "menu.about" })}</button>
    </div>
  );
};
