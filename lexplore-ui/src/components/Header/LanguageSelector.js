  ...
  return (
    ...
    >
      ...
      >
        <GrLanguage />
        <p>
          {SUPPORTED_LANGUAGES.filter(({ code }) => code === locale)[0].label}
        </p>
      </button>
      {openDropdown && (
        <div className={styles.languageSelectorOptionsWrapper}>
          {SUPPORTED_LANGUAGES.filter(({ code }) => code !== locale).map(
            ({ code, label }) => {
              return (
                <button
                  className={`${styles.languageOptionButton} ${styles.buttonColors}`}
                  key={code}
